...
async function main() {
    ...
    await prisma.user.create({
        data: { email: "admin@adun.edu.ng", password: hashedPassword, firstName: "System", lastName: "Administrator", role: "ADMIN", department, isActive: true }
    });
    ...
    for (const l of lecturersData) {
        const lec = await prisma.user.create({ data: { ...l, password: hashedPassword, role: "LECTURER", department, isActive: true } });
        lecturers.push(lec);
    }
    ...
    const coursesData = [
        ...
        { code: "SWE 202", title: "Software Requirements Engineering", level: 200, unit: 3, semester: "First" },
        ...
        { code: "SWE 303", title: "Database Systems for SWE", level: 300, unit: 3, semester: "Second" },
    ...
    for (let i = 0; i < coursesData.length; i++) {
        ...
        const mod = await prisma.module.create({ data: { title: "Module 1: Fundamentals", order: 1, courseId: course.id } });
        await prisma.content.create({
            data: { title: "Course Overview Lecture", type: "VIDEO", url: "https://example.com/video.mp4", duration: 60, order: 1, moduleId: mod.id }
        });
        await prisma.content.create({
            data: { title: "Reading Material", type: "DOCUMENT", url: "https://example.com/reading.pdf", duration: 0, order: 2, moduleId: mod.id }
        });
    ...
    for (const student of students) {
        ...
        for (const course of selectedCourses) {
            ...
            if (course.semester === "First") {
                ...
                let gradeLetter = "F"; let gp = 0;
                if (total >= 70) { gradeLetter = "A"; gp = 5; }
                else if (total >= 60) { gradeLetter = "B"; gp = 4; }
                else if (total >= 50) { gradeLetter = "C"; gp = 3; }
                else if (total >= 45) { gradeLetter = "D"; gp = 2; }
                else if (total >= 40) { gradeLetter = "E"; gp = 1; }
